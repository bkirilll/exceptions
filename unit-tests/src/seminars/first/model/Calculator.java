...
public class Calculator {
    ...
    public static double calculatingDiscount(double purchaseAmount, int discountAmount) {
        if (discountAmount < 0) {
            throw new ArithmeticException("discountAmount cant be less then zero");
        } else if (discountAmount > 100) {
            throw new ArithmeticException("discountAmount cant be more then 100");
        }
        return purchaseAmount*(discountAmount/100.0);
    }
}